...
--bordureY =0
  ...
  QS192 = iniQS192()
  QS96 = iniQS96()
  imagePerso = { sprite = love.graphics.newImage("sprites/fille.png"), frame = QS96.AV[1]}
  ...
  if love.filesystem.exists('game.sav') == false then
    ...
    --love.filesystem.write('game.sav', Tserial.pack(objectToSave) )
    gameInfos = Tserial.unpack( love.filesystem.read( 'game.sav' ) )
    perso = gameInfos.perso
    posMap = gameInfos.posMap
  end
  imgMenu = love.graphics.newImage("img/menu.png")
  mapTable = require('map/m'..perso.localisation..posMap.x..'_'..posMap.y) -- on charge la bonne map
  tileset = tilesets[mapTable.tileset]
...
function love.draw()
  ...
  love.graphics.draw(imgMenu, 0+bordureX, 0+bordureY)
